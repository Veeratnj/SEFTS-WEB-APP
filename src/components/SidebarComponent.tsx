...
const SidebarComponent: React.FC = () => {
  const [nseStocks, setNseStocks] = useState([
    { name: 'RELIANCE', points: 12.34, percentage: 1.23, token: '', trend_type: '' },
    { name: 'TCS', points: -8.56, percentage: -0.87, token: '', trend_type: '' },
    { name: 'INFY', points: 5.21, percentage: 0.45, token: '', trend_type: '' },
    { name: 'HDFC', points: -3.87, percentage: -1.12, token: '', trend_type: '' },
    { name: 'SBIN', points: 0.45, percentage: 0.18, token: '', trend_type: '' },
  ]);

  const [optionsStocks, setOptionsStocks] = useState([
    { name: 'NIFTY 50', points: 15.22, percentage: 1.45, token: '', trend_type: '' },
    { name: 'BANKNIFTY', points: -7.14, percentage: -0.56, token: '', trend_type: '' },
    { name: 'FINNIFTY', points: 4.67, percentage: 0.72, token: '', trend_type: '' },
  ]);
  ...
  const [selectedFilter, setFilter] = useState<'All' | 'Bullish' | 'Bearish'>('All');
  const filteredFavs = favs.filter(favName => {
    const stock = stocks.find(s => s.name === favName);
    if (!stock) return false;
    if (selectedFilter === 'All') return true;
    if (selectedFilter === 'Bullish') return stock.trend_type === 'bullish';
    if (selectedFilter === 'Bearish') return stock.trend_type === 'bearish';
    return false;
  });

  const filteredStocks = searchTerm
    ? stocks.filter(stock => stock.name.toLowerCase().includes(searchTerm.toLowerCase()))
    : [];
  ...
  return (
    <div className="bg-gradient-to-b from-blue-50 to-white rounded-2xl shadow-lg p-6 h-full flex flex-col">
      ...
      <div className="text-center mb-6">
        <h2 className="text-2xl font-bold text-blue-700 tracking-wide">SMART ELITE TRADING CLUB</h2>
      </div>
        ...
        <div className="mt-4">
          ...
          <div className="flex justify-center space-x-4 mb-4">
            {['All', 'Bullish', 'Bearish'].map(filter => (
              <button
                key={filter}
                onClick={() => setFilter(filter as 'All' | 'Bullish' | 'Bearish')}
                className={`px-4 py-2 rounded-full font-semibold text-sm shadow cursor-pointer ${
